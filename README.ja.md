...
| `backprop ui --share` が拒否される | `INPUT_AUTH_REQUIRED` | `--auth user:password` を指定してください。v1.2.0（GHSA-f65r-h4g3-3h9h）以降、`--share` を `--auth` なしで使用することはハードエラーとなり、オプトアウトはできません。認証情報を公開できない場合は SSH ポートフォワーディングを使用してください。 |
