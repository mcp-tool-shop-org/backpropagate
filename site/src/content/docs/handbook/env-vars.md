...
| `BACKPROPAGATE_TRAINING__METHOD` | `sft` | **v1.5 (orpo) / v1.6 (simpo, kto)** — training objective. One of `sft` / `orpo` / `simpo` / `kto`. `sft` (default) = supervised fine-tuning. `orpo` and `simpo` = reference-free preference tuning (need a paired `{prompt, chosen, rejected}` dataset; single-stage, no reference model). `kto` = Kahneman-Tversky Optimization on **unpaired** `{prompt, completion, label}` binary-feedback data (LoRA-only). Any other value is rejected with `CONFIG_INVALID_SETTING`. See [Preference tuning](/backpropagate/handbook/preference-tuning/). |
| `BACKPROPAGATE_TRAINING__ORPO_BETA` | `0.1` | **v1.5** — ORPO odds-ratio weight (lambda). Keep > 0. Ignored unless `METHOD=orpo`. |
| `BACKPROPAGATE_TRAINING__SIMPO_BETA` | `2.0` | **v1.6** — SimPO reward-scaling temperature (`beta` in TRL's `CPOConfig` under `loss_type="simpo"`). The cross-setup safe floor from the SimPO paper ([arXiv:2405.14734](https://arxiv.org/abs/2405.14734)). Any finite value is admissible; the `gamma/beta` ratio is what carries the degeneration risk (warned, not blocked). Ignored unless `METHOD=simpo`. |
| `BACKPROPAGATE_TRAINING__SIMPO_GAMMA` | `1.0` | **v1.6** — SimPO target reward margin (`gamma`; absolute, = `beta`×0.5 at the default beta). Must be > 0 — a non-positive margin degenerates the objective (`CONFIG_INVALID_SETTING`). A `gamma/beta` ratio above 1.0 risks repetitive output and emits a WARN (not an error). Ignored unless `METHOD=simpo`. |
| `BACKPROPAGATE_TRAINING__KTO_BETA` | `0.1` | **v1.6** — KTO loss temperature (`beta` in TRL's `KTOConfig`; the KTO paper / TRL default, [arXiv:2402.01306](https://arxiv.org/abs/2402.01306)). Ignored unless `METHOD=kto`. |
| `BACKPROPAGATE_TRAINING__KTO_DESIRABLE_WEIGHT` | `1.0` | **v1.6** — loss weight on **desirable** (`label=true`) KTO examples. Must be > 0 (`CONFIG_INVALID_SETTING`). This is the operator's **starting point**: the trainer auto-rebalances the effective desirable/undesirable weights from your label counts toward the `[1:1, 4:3]` band, so the final ratio is data-derived, not this raw value. Ignored unless `METHOD=kto`. |
| `BACKPROPAGATE_TRAINING__KTO_UNDESIRABLE_WEIGHT` | `1.0` | **v1.6** — loss weight on **undesirable** (`label=false`) KTO examples. Must be > 0 (`CONFIG_INVALID_SETTING`). Auto-rebalanced from label counts alongside `KTO_DESIRABLE_WEIGHT` (see above). Ignored unless `METHOD=kto`. |
