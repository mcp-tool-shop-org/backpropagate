...
| `RUNTIME_FULL_FT_MODEL_TOO_LARGE` | `Trainer(mode="full")` was requested for a model whose parameter count exceeds the documented 4B full-fine-tuning ceiling. Fires at `Trainer.__init__` (preset-table lookup) AND a second time at `Trainer.load_model()` (authoritative `model.num_parameters()` check — this second gate is what catches the marketed "3B" models, whose true count is 3.08–3.24B). | Two recoveries: (1) re-run with `mode="lora"` (the default) — LoRA fits 7B+ on a 16GB card and is the recommended path per [Biderman 2024](https://arxiv.org/abs/2405.09673) + [Thinking Machines 2025](https://thinkingmachines.ai/blog/lora/); (2) switch to a model under the ceiling — `smollm3-3b`, `qwen2.5-3b`, `llama-3.2-3b`, or `llama-3.2-1b` fit a 16GB card, while `phi-4-mini-3.8b` and `qwen3.5-4b` also support `mode="full"` but need a 24GB+ card. See [full fine-tuning](/backpropagate/handbook/full-fine-tuning/) for the quality math and the recovery decision tree. |
